...
const OUT = '/home/user/Demo/progress/records/cone0';
...
const ARMS = [
  ...
  /* EXPLORATORY, base only. No candidate arm exists here on purpose: this capture exists to show
     where the cone actually falls on the shipped camera so an ROI can be placed on evidence, and
     seeing the candidate first would let me place that ROI where the candidate happens to win. */
  { id: 'base', toward: 0.35 },
  { id: 'restore', toward: 0.35 },     // determinism check, still worth having
];

const BOOT_ID = randomUUID().slice(0, 12);

const report = {
  prereg: 'NONE — exploratory, base only, scores nothing',
  runner: 'progress/records/cone0.mjs',
  bootId: BOOT_ID,
