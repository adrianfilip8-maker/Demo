/**
 * bandgate.mjs — PREREG-linchroma's runner. 4 frames, one chunk, NO src change.
...
const OUT = path.join(ROOT, 'progress/records/bandgate1');
...
await withGame({ width: 1280, height: 720, quality: 'high', timeout: 900000, onLocked },
  async ({ page, info }) => {
    ...
    writeFileSync(path.join(OUT, 'manifest.json'), JSON.stringify({
      seal: 'PREREG-bandgate', head: HEAD, srcHash: EXPECT_SRC, warmup: WARMUP,
      shot: SHOT, expectRows: 4, capturedAt: new Date().toISOString(), rows,
    }, null, 2));
  });
console.log(`\n${rows.length} frames -> ${OUT}`);
console.log('score with: node progress/records/bandgate/bandgate-score.mjs');
