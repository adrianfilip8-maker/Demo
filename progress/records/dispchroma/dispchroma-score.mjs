/**
 * PREREG-dispchroma §5-§8 — the registered scorer. Reads progress/records/dispchroma1/ and prints
...
const DIR = process.env.DISPCHROMA_DIR || path.join(ROOT, 'progress/records/dispchroma1');
...
const CLIP_MAX = 0.10;                             // §7 CLIP_traversal — the bar this seal turns on
...
/* ── PF_COSTUME (§5, NEW) — is the rect on the COSTUME, not merely on the subject? ─────────
   litbleach's E_H_combat read 348.5 deg while PF_MASK read 99.6% SUBJECT. 99.6% subject is not
   99.6% costume: a rect on face, gloves or trim is subject-masked, off-hue, and low-chroma —
   which also gates the hold off (NOTE-litbleach2-aim.md). A subject mask cannot catch that; a
   hue gate can. */
for (const shot of ROSTER) {
  const m = M[shot]?.off;
  const d = m ? hueDelta(m.H, REF_HUE) : null;
  report.push(`PF_COSTUME ${shot.padEnd(9)} off H ${m ? m.H.toFixed(1) : '—'}  |dH| ${d === null ? '—' : d.toFixed(1)} <= ${PF_HUE_TOL}`);
  guards[`PF_COSTUME_${shot}`] = d === null ? null : d <= PF_HUE_TOL;
}
...
const PREFLIGHT_OK = guards.PF_STAGE === true
  && ROSTER.every((s) => guards[`PF_COSTUME_${s}`] === true)
...
/* ── CLIP (§7) — the bar this seal turns on. A pixel driven to the gamut wall by the hold has
   lost its shading information, which is §2.1's banded cel shading gone in those pixels and the
   "flat sticker" the §9 LOOK exists to catch. Counted only where the hold PUT it there: at the
   wall in `on` and not already at the wall in `off`. */
for (const shot of DOSE) {
  const o = img(row(shot, 'off')), n = img(row(shot, 'on'));
  let clipped = 0, tot = 0;
  if (o && n) {
    const mask = litMask(o, RECT[shot]);
    tot = mask.length;
    for (const i of mask) {
      const a = i * o.ch, b = i * n.ch;
      const wasWall = o.data[a] >= 255 || o.data[a + 1] >= 255 || o.data[a + 2] >= 255
        || o.data[a] <= 0 || o.data[a + 1] <= 0 || o.data[a + 2] <= 0;
      const isWall = n.data[b] >= 255 || n.data[b + 1] >= 255 || n.data[b + 2] >= 255
        || n.data[b] <= 0 || n.data[b + 1] <= 0 || n.data[b + 2] <= 0;
      if (isWall && !wasWall) clipped++;
    }
  }
  const frac = tot ? clipped / tot : null;
  report.push(`CLIP_${shot.padEnd(9)} ${frac === null ? '—' : (100 * frac).toFixed(1) + '%'} of measured px driven to the gamut wall (want <= ${100 * CLIP_MAX}%)`);
  guards[`CLIP_${shot}`] = gated(frac === null ? null : frac <= CLIP_MAX);
}
