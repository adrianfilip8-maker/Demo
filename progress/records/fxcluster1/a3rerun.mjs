/**
 * a3rerun — PREREG-fxcluster-a3: sub-arm A third letter. Same −0.20 heading lever as both
 * parents; what changes is the INSTRUMENT (re-sited onto the guard's ground pool, chosen by
 * the measured E/N in a3-choose.json) and the CAPTURE CLOCK (engine.time pinned per arm).
 *
 * Why the clock pin (seal §0): src/ai/Guard.js carries THREE absolute-engine-time terms —
 * :1588 the CPU flicker (+-9%), :278-279 BEAM_FRAG dust (+-19%), :347 POOL_FRAG (+-13.6%) —
 * and the cone is the one animated subsystem that never got the shot-event clock re-base
 * Particles.js:2600-2612 and Lighting.js:568-571 both perform and both document. Pinning
 * engine.time at the head of every arm pins all three at once; Particles and Lighting re-base
 * AFTER that point, so their relative clocks are untouched. No src edit: engine.time is a
 * plain property (Engine.js:87) and Debug.step(n,0) is documented to freeze it (Debug.js:152).
 *
 * The a2/c2 pool wipe is KEPT: a2 measured it removing the other, real contaminant
 * (base->base2 2.06 -> 1.36). Both mechanisms are addressed, each by the thing that fits it.
 *
 * NO SRC EDITS: verifies the committed Guard.js heading seam + Particles.js poke path and
 * aborts if absent. Frames + readback INCREMENTALLY, a3-prefixed. Idempotent resume. FIFO
 * lock via withGame (an sbs3 capture holds it at sealing — queue politely). Scoring offline:
 * a3score.mjs. No git — the coordinator sweeps.
 */
...
/* PREREG-fxcluster-a3 §1: the pinned engine clock. A constant above any natural boot value so
...
const FRAME = (arm) => path.join(OUT, `a3-guard.${arm}.png`);

function makeReport() {
  const report = {
    prereg: 'PREREG-fxcluster-a3.md',
    chunk: 'a3',
    tPin: T_PIN,
    startedAt: new Date().toISOString(), srcTreeBefore: treeHash(), arms: [],
  };
  const save = () => writeFileSync(path.join(OUT, 'a3-readback.json'), JSON.stringify(report, null, 1));
  save();
  return { report, save };
}
...
async function chunkA3() {
  const frames = ['base', 'base2', 'cand', 'restore'].map(FRAME);
  if (frames.every(existsSync)) { log('chunk a3: all frames present — skipping (idempotent resume)'); return; }
  const { report, save } = makeReport();
  log(`chunk a3 (guard only): srcTree ${report.srcTreeBefore}, tPin ${T_PIN} — booting (FIFO lock; queueing politely)`);
  await withGame({ width: 1280, height: 720, quality: 'high', timeout: 60 * 60 * 1000 }, async ({ page, info }) => {
    ...
    log(`  chunk a3 done — srcTree after ${report.srcTreeAfter} (${report.srcTreeAfter === report.srcTreeBefore ? 'STABLE' : 'MOVED — flag in RESULT'})`);
  });
  log('chunk a3: lock released');
}
