/**
 * c3rerun — PREREG-fxcluster-c3: same warmer block, re-anchored instrument (see seal §0).
 *
...
const FRAME = (arm) => path.join(OUT, `c3-combat.${arm}.png`);

function makeReport() {
  const report = {
    prereg: 'PREREG-fxcluster-c3.md',
    chunk: 'c3',
    startedAt: new Date().toISOString(), srcTreeBefore: treeHash(), arms: [],
  };
  const save = () => writeFileSync(path.join(OUT, 'c3-readback.json'), JSON.stringify(report, null, 1));
  save();
  return { report, save };
}
...
async function chunkC3() {
  const frames = ['base', 'cand', 'restore'].map(FRAME);
  if (frames.every(existsSync)) { log('chunk c3: all frames present — skipping (idempotent resume)'); return; }
  const { report, save } = makeReport();
  log(`chunk c3 (combat only): srcTree ${report.srcTreeBefore} — booting (own FIFO lock hold; skyswirl/mradius may be ahead — queueing politely)`);
  await withGame({ width: 1280, height: 720, quality: 'high', timeout: 60 * 60 * 1000 }, async ({ page, info }) => {
    ...
    await armCapture(page, report, save, 'cand', { poke: armPoke, pokeArg: { mode: 'cand', block: C3_POKE }, probe: probeCombat });
    ...
    log(`  chunk c3 done — srcTree after ${report.srcTreeAfter} (${report.srcTreeAfter === report.srcTreeBefore ? 'STABLE' : 'MOVED — flag in RESULT'})`);
  });
  log('chunk c3: lock released');
}
