 ...
 * Consumes off / bon / blamp / back rows of progress/records/guardpass1/ (abon is LOOK
 * material only). VOID is not PASS; ship = every gate row PASS + the binding LOOK.
 */
import {
...
/* ── verdict ────────────────────────────────────────────────────────────────────────────── */
console.log('── guardcone-score — PREREG-guardcone §3 ──');
for (const line of report) console.log('  ' + line);
const v = shipVerdict(guards);
console.log('\nbars:');
for (const [k, s] of Object.entries(v.states)) console.log(`  ${k.padEnd(20)} ${s}`);
console.log('\n' + verdictLine(v, 'the cone tuple (coneShape 1 + colPatrol #ffd9a0 + core 0.62 + lampToon 1.0 …) — LOOK gate still binding (PREREG-guardcone §8.3)'));
