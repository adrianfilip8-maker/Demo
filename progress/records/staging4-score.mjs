/**
 * staging3-score — scores PREREG-staging4.md's registered quantities on the frames staging3.mjs
 ...
 *   1. base gates live on the FIGURE-COLUMN family (P1/P2/P3/P7/figure medL), which four boots
 *      show reproduces to 0.6% or better, instead of the guard-mass rect, which moves 16% between
 *      boots and so could never hold a +-6% band (seal §4.1, §198.1);
 ...
 *   4. THREE discarded prerolls absorb compile AND the early-boot state transition
 *      `staging4-floor` found; P-F9 requires all three present and same-boot;
 *   5. self-calibration against committed sbs3/guard.png before anything is scored (§122.1).
 *
 ...
 *
 *   usage: node progress/records/staging4-score.mjs
 */
...
/* ===== SEAL CONSTANTS (PREREG-staging4 §4 — filled from staging3-derive-analyze at seal time,
   committed BEFORE the scored capture boots) ===== */
/* §4.1 — base gates on the figure-column family, which four boots show reproduces (P1 spread
   0.6%, P2 and P7 exact, P3 0.5%, figure medL 0.003) where the guard-mass rect moves 16% between
   boots. Bands are the observed four-boot range plus a margin of about its own width. */
const BASE_GATES = {
  P1: [15.4, 16.5],
  P2: [300, 312],
  P3: [88.5, 91.0],
  P7: [32, 34],
  R6_retired_figureRectMedL: [22.7, 23.7],
};
/* REPORTED, gating nothing: boot-dependent by 16% and 2.7 L respectively (§198.1). */
const REPORT_ONLY = ['R2_guardMassRect_medL', 'G_doorwayPool_medL'];
const PF4_CEIL = 0;                    // frame-wide, unchanged — staging4-floor says it is affordable
/* ================================================================================== */
...
if (!BASE_GATES || PF4_CEIL === null) {
  console.log('SEAL CONSTANTS UNFILLED — this scorer must not run before PREREG-staging4 §4 is final.');
  process.exit(2);
}
...
const out = { prereg: 'PREREG-staging4.md', at: new Date().toISOString(), calibration: cal, arms: S, verdicts: {} };
...
{
  const PR = ['preroll1', 'preroll2', 'preroll3'];
  const missing = PR.filter((p) => !existsSync(files[p]));
  const scoredIds = rb ? new Set((rb.arms || []).filter((a) => !a.discard && present.includes(a.arm)).map((a) => a.bootId)) : new Set();
  const wrongBoot = PR.filter((p) => byArm[p] && scoredIds.size && !scoredIds.has(byArm[p].bootId));
  const ok = !missing.length && !wrongBoot.length;
  console.log(`  P-F9 three prerolls present: ${missing.length ? 'NO -> missing ' + missing.join(', ') : 'YES'}; same bootId: ${wrongBoot.length ? 'NO -> ' + wrongBoot.join(', ') : 'YES'}  -> ${ok ? 'ok' : 'VOID'}`);
...
console.log('\n=== BASE GATES (P-F3 — VOID, not FAIL; figure-column family, §4.1) ===');
let voided = false;
if (S.base) {
  for (const [k, [lo, hi]] of Object.entries(BASE_GATES)) {
    const v = S.base[k], ok = v >= lo && v <= hi; if (!ok) voided = true;
    console.log(`  ${ok ? 'ok  ' : 'OUT '} base ${k.padEnd(26)} = ${String(v).padStart(8)}  band [${lo}, ${hi}]`);
  }
  console.log('  REPORTED, gating nothing (boot-dependent — §198.1):');
  for (const k of REPORT_ONLY) console.log(`       base ${k.padEnd(26)} = ${S.base[k]}`);
