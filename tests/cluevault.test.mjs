...
test('V1b the imported bottle is the size the placements were tuned around (§698)', async () => {
  /**
   * The bottle is no longer hand-authored — it is a bake of the reference project's `BOTTLE.glb`,
   * regenerated by `tools/godot2bottle.mjs`. That makes its size a **build output**, and R1/R2/R3
   * below are tuned against it: `TUNE.clueCollect` is documented as "playerRadius 0.34 + half the
   * bottle's height", and the whole 2.40 m magnet argument is about a bottle standing where it
   * was put. A re-bake at a different scale, or a normalisation that stopped putting the base at
   * the origin, would move the object out from under all three without touching a single line
   * any of them read.
   *
   * So this pins the two properties those tests actually depend on, and nothing else — not the
   * vertex count, not the colours, not the profile, all of which are the asset's business and are
   * meant to change when the asset does.
   *
   *   HEIGHT  0.43260 m at the shipped `h`. Not a chosen number: it is the measured height of the
   *           procedural lathe this replaced (`h * 0.95` body, cork placed at `h * 0.93` reaching
   *           `h * 1.03`), so matching it is what makes the substitution invisible to placement.
   *   BASE    `bbox.min.y === 0`. Both meshes are base-origin, which is the reason the pickup
   *           point does not move; a mesh centred on its middle would sink half its height.
   *
   * DOMAIN (§418.3)
   * PASSES ON: the shipped bake — 0.43260 m, base at 0.
   * FAILS ON:  RUN in-arm — the same geometry rebuilt at a deliberately wrong `h` misses the
   *            height band, so the tolerance is shown to be able to reject rather than assumed to.
   */
  const { clueBottle, mergeAll, CLUE_ATTRS } = await import('../src/world/PropKit.js');
  ...
  assert.ok(Math.abs(height - 0.43260) < 0.0005,
    `the clue bottle now stands ${height.toFixed(5)} m, not the 0.43260 m the twelve placements ` +
    'and TUNE.clueCollect were tuned around — re-derive R1/R2/R3 before changing this number');
  assert.ok(Math.abs(bb.min.y) < 1e-6,
    `the clue bottle's base is at y ${bb.min.y.toFixed(6)}, not 0 — it is no longer base-origin, ` +
  ...
  /* The discriminator: the band must be able to fail, or "0.43260" is decoration. */
  const wrong = measure(PTUNE.clueHeight * 1.5);
  assert.ok(Math.abs((wrong.max.y - wrong.min.y) - 0.43260) > 0.0005,
...
test('V13 the clue toast draws a bottle, not the unknown-icon fallback', async () => {
   ...
   *
   * ── The colour arm, RE-DERIVED rather than re-pinned (§698) ──────────────────────────────
