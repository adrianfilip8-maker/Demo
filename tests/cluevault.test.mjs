...
test('V1b the clue bottle is the size the placements were tuned around (§700, re-derived §701)', async () => {
  /**
   * The bottle is not hand-authored — it is a bake of the reference project's `BOTTLE.glb`,
   * regenerated by `tools/godot2bottle.mjs`. That makes its size a **build output**, and R1/R2/R3
   * below rest on it: the whole 2.40 m magnet argument is about a bottle standing where it was
   * put. A re-bake at a different scale, or a normalisation that stopped putting the base at the
   * origin, would move the object out from under all three without touching a single line any of
   * them read.
   *
   * ── RE-DERIVED, not re-pinned (§701) ──────────────────────────────────────────────────────
   * This arm used to assert one literal — `0.43260 m` — and the bottle has since been tripled on
   * request. Bumping that literal to 1.29780 and moving on would have repeated exactly the
   * mistake §700 corrected in V13: **the literal was never the claim, it was the claim's value on
   * the day.** The claim is a chain, and the chain is what is asserted now:
   *
   *   BAKE       `clueBottle({ h: 1 })` stands `CLUE_HEIGHT_RATIO` and its base is at y 0. This
   *              is the property of the MODULE, and the only one a re-bake can silently break.
   *   SIZE       one authored constant, `PropKit.CLUE_HEIGHT`, and `TUNE.clueHeight` is it.
   *   DELIVERED  `h * CLUE_HEIGHT_RATIO` = 1.29780 m — the number the twelve placements and
   *              `tools/bottlefit.mjs`'s clearance figures were measured against. Still pinned
   *              absolutely, because a placement cannot check itself: change it and R1/R2/R3 and
   *              the interpenetration sweep must be re-run, not re-read.
   *   DERIVED    `clueCollect` and `clueSway` are documented as functions of the DELIVERED
   *              height, and both are checked against it here. Before §701 this was prose; a
   *              scale change that moved the mesh and left the radii was a silent regression
   *              with no assertion anywhere in the tree standing in front of it.
   *   TWIN       `Props._clueBottles()` builds the same mesh at the same size. `Pickups` hides
   *              whichever of the two it does not own, so a half-applied scale edit looks
   *              perfect in every frame and leaves the built world disagreeing with itself.
   *
   * DOMAIN (§418.3)
  ...
  /* ---- BAKE: the module is unit height, base at origin ---- */
  const unit = measure(1);
  const unitH = unit.max.y - unit.min.y;
  assert.ok(Math.abs(unitH - CLUE_HEIGHT_RATIO) < 5e-4,
    `clueBottle({h:1}) stands ${unitH.toFixed(5)} m against CLUE_HEIGHT_RATIO ${CLUE_HEIGHT_RATIO} — ` +
    'the baked module is no longer unit height, so every size derived from `h` is wrong by that factor');
  assert.ok(Math.abs(unit.min.y) < 1e-6,
    `the baked module's base is at y ${unit.min.y.toFixed(6)}, not 0 — it is no longer base-origin, ` +
    'so every bottle now floats or sinks by that much against its authored spot');

  /* ---- SIZE: one constant, and TUNE reads it ---- */
  assert.equal(PTUNE.clueHeight, CLUE_HEIGHT,
    'TUNE.clueHeight has drifted from PropKit.CLUE_HEIGHT — the bottle has two authored sizes again');
  ...
  /* ---- DERIVED: the two radii that are functions of the delivered height ---- */
  const wantCollect = PTUNE.playerRadius + height / 2;
  assert.ok(PTUNE.clueCollect <= wantCollect + 1e-9 && PTUNE.clueCollect > wantCollect - 0.015,
    `TUNE.clueCollect is ${PTUNE.clueCollect} against a derivation of playerRadius ` +
    `${PTUNE.playerRadius} + half the delivered height ${(height / 2).toFixed(4)} = ` +
    `${wantCollect.toFixed(4)}. The bottle has been rescaled and the CONTACT radius has not: ` +
    'the player walks through the mesh before it registers.');
  assert.ok(PTUNE.clueCollect < PTUNE.magnet,
    'the clue contact radius has grown past the magnet — nothing is ever pulled, it is only snapped');
  assert.ok(Math.abs(PTUNE.clueSway - height / 7) < 0.0005,
    `TUNE.clueSway is ${PTUNE.clueSway} against the reference's 1/7-of-height proportion, ` +
    `${(height / 7).toFixed(4)} — the bottle has been rescaled and its sway has not, which is ` +
    '§700\'s own warning ("copying 0.125 onto a bottle half the size") in the other direction');

  /* ---- TWIN: PROPS builds the same size, and does it by reading the constant ---- */
  const twin = /clueBottle\(\{\s*h:\s*([A-Za-z_$][\w$]*|[\d.]+)/.exec(stripComments(PROPS_SRC));
  assert.ok(twin, 'Props._clueBottles() no longer builds a clueBottle — the decorative twin has gone');
  assert.equal(twin[1], 'CLUE_HEIGHT',
    `Props builds its clue bottle at \`${twin[1]}\` rather than at PropKit.CLUE_HEIGHT. The twin ` +
    'and the pickup are the same object at the same twelve spots and Pickups hides one of them, ' +
    'so a scale that reaches one file and not the other is invisible in every frame.');

  /* The colour stream the merge is required to preserve; without it the bottle draws white and
     nothing else in this file would notice. */
  const geo = (() => { const p = []; clueBottle({ h: PTUNE.clueHeight }).drain((_k, g) => p.push(g)); return mergeAll(p, CLUE_ATTRS); })();
  assert.ok(geo.attributes.color, 'the merged clue bottle carries no colour attribute — CLUE_ATTRS ' +
    'has been dropped at the builder or the merge, and the bottle draws flat white');
...
test('V13 the clue toast draws a bottle, not the unknown-icon fallback', async () => {
   ...
   *
   * ── The colour arm, RE-DERIVED rather than re-pinned (§700) ──────────────────────────────
