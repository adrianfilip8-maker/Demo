 ...
 * `globalThis.__ANIM_AB` for a runner that boots the page itself. An unknown, misspelled or absent
 * token resolves to `proc` — **the default is unchanged, and restoring it is the absence of a
...
export function buildClipSet(raw) {
  ...
  const fill = regime !== 'mixamo-pure';
  const used = new Set();
  for (const [game, src] of Object.entries(MIXAMO_ALIAS)) {
    const raw = MIXAMO_CLIPS[src];
    if (!raw || !CLIPS[game]) continue;           // a regenerated module that dropped a clip degrades
    table[game] = spliceClip(game, raw, CLIPS[game], fill);
    origin[game] = `mixamo:${src}`;
    used.add(src);
  }
  return { table, origin, regime, unused: Object.keys(MIXAMO_CLIPS).filter((n) => !used.has(n)) };
}
