/**
 * §D2 / PREREG-bodyhue4.md — scorer. Thresholds transcribed from the seal, not re-derived.
 *
 ...
 *   CAL-3  DELETED by PREREG-bodyhue4 §2 — the circular median makes wraparound moot
 ...
 * with the two-shot limit stated alongside it (PREREG-bodyhue4 §3).
 */
import { readFileSync } from 'node:fs';
import { shipVerdict, verdictLine, guardState, PASS, VOID } from './gate.mjs';

const DIR = process.env.SANDS_OUT || 'shots/bodyhue4';
...
for (const r of rows) {
  ...
  console.log(`  ${r.shot.padEnd(13)} ${String(r.n).padStart(7)}  ${(100 * r.cov).toFixed(2).padStart(5)}   `
    ...
    + `${r.straddle ? '  (linear span wraps; circular median used)' : ''}`);
}

const usable = rows.filter((r) => r.hueA != null && r.hueB != null);
...
/* CAL-3 DELETED by PREREG-bodyhue4 §2: the circular median makes wraparound a non-issue, so the
   guard has nothing left to protect. Straddle is still reported by the runner, informationally. */
...
const cals = [cal1, cal2].map(guardState);
let outcome;
