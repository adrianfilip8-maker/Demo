...
const fmt = (tag, c) => {
  console.log(`  ${tag.padEnd(26)} shoulderW ${c.shoulderW.toFixed(3)} m   hand sep ${String(c.sepCm).padStart(6)} cm (${c.sepShoulders} sh)`);
  for (const S of ['L', 'R']) {
    const a = c[S];
    console.log(`      ${S}: abduction ${String(a.abduction).padStart(6)}°   flexion ${String(a.flexion).padStart(6)}°`
