/**
 * §D2 / PREREG-bodyhue4.md — same-boot, edges excluded, CIRCULAR median.
 *
...
const OUT = process.env.SANDS_OUT || 'shots/bodyhue4';
...
for (const shot of SHOTS) {
  ...
  /* PREREG-bodyhue4 §2 — CIRCULAR median. A linear median over an angle is invalid when the set
     wraps, which is what voided run 3 on 131 pixels out of 19,969. Rotate the sample so its mean
     resultant direction sits at 180 degrees, take the linear median there, rotate back: exact for
     a unimodal sample and standard for angular data. CAL-3 is gone because this makes it moot. */
  const med = (a) => {
    if (!a.length) return null;
    let sx = 0, sy = 0;
    for (const h of a) { const r = h * Math.PI / 180; sx += Math.cos(r); sy += Math.sin(r); }
    const mean = Math.atan2(sy, sx) * 180 / Math.PI;              // mean resultant direction
...
console.log('\nscore with: node tools/bodyhue4score.mjs');
