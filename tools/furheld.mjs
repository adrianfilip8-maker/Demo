/**
 * charread.mjs — the two tests critic pass 5 §3.1 names, made scoreable offline.
 *
 *   T40: "the tail must read as one clean shape at 40 px."
 *   M5 : "the mask ... cannot be allowed to vary per pose — verify it in the same five poses."
 *
 * WHAT THIS IS, AS THE GAP (KNOWN_ISSUES §11) — the suffix between this number and the frame:
 *   - NO cel ramp, no key/fill/rim, no PostFX, no AgX. Pixels are **material albedo** (PAL),
 *     flat. Every read below is therefore an albedo read. That is the right layer for the two
 *     claims being scored ("pale lavender tube with navy plates", "the mask is half-absent"
 *     are both shape+albedo reads) and the wrong layer for any claim about contrast in frame.
 *   - The inverted-hull ink shell IS implemented, as a normal-expanded black prepass at the
 *     shot's own px/m (Outline.js's rule: ~2.5 px on screen). This is not decoration: the
 *     critic's "each plate carries its own black outline" IS the shell, and a tool without it
 *     cannot see the defect it is being pointed at. It is an approximation — real Outline.js
 *     runs per-mesh with backface culling, this expands every triangle of one merged mesh —
 *     so it OVERSTATES ink at internal creases and matches it at true silhouette edges.
 *   - NO foot IK (Rig.footIK). Nothing below the knee is the drawn pose. No band here reads
 *     feet.
 *   - NO level, so no occlusion by ledge/wall. `hero`'s ledge really does cut the figure.
 *   - Head/tail/mask all sit above those cuts, which is why those are the bands scored.
 *
 * PROVING THE INSTRUMENTS ON KNOWNS (§33 — a band must be reachable from both sides). Run
 * `--selftest`: synthetic clean tube, synthetic sawtooth tube, mirrored mask, half-erased mask.
 * Every metric below prints its control pair, and a metric whose two controls do not straddle
 * the decision threshold is reported as NON-DISCRIMINATING and its verdict is withheld.
 *
 *   node charread.mjs --shot sly-closeup [--rows 720] [--root /path] [--out dir] [--tag t]
 *   node charread.mjs --mask5            # M5 across the five character shots
 *   node charread.mjs --selftest
 */
...
function maskRead(idBuf, W, H, IDS) {
  ...
  return {
    ...
    bandMed: medBand, bandFrac: fracBand,
  };
}
...
/* SlyModel GROUPS order; 8/9/10 are the furTuft* card groups carrying TUNE.tuftInk. Read off the
   built materials rather than hardcoded, so this cannot drift from the model. */
const INKW_BY_MAT = (() => {
  const mats = Array.isArray(sly.mesh.material) ? sly.mesh.material : [sly.mesh.material];
  return mats.map((m) => { const w = m?.userData?.outline; return Number.isFinite(w) ? w : 1; });
})();
...
function raster({ shot, region = 'all', dropTuft = null, ink = true, scalePx = null, azOverride = null }) {
  ...
  /* Per-material ink weight, mirroring Outline.js's `applyInkWeights` -> `slyInk` stream.
     The real shader multiplies the hull thickness by this; here it scales the normal expansion,
     which is the same operation. Indices track SlyModel's GROUPS array. Without this the tool
     is blind to TUNE.tuftInk by construction and would report the change as a null. */
  const INKW = INKW_BY_MAT; if (!globalThis.__inkwShown) { globalThis.__inkwShown = 1; console.log('INKW_BY_MAT =', JSON.stringify(INKW), 'matIsArray=', Array.isArray(sly.mesh.material)); }
  const drawPass = (off, forceId, backOnly = false) => {
    for (const [a, b, c, m] of tris) {
      const off_ = off * (INKW[m] ?? 1);
      const A = toPx(view(a, off_)), B = toPx(view(b, off_)), C = toPx(view(c, off_));
...
/* THREE-ARM HOLD-OUT (see KNOWN_ISSUES SlyModel.js:499-507).
   Arm A = shipped (every family on). Arm B = TAIL ONLY (every on-face family suppressed).
   Arm C = no tufts at all. B is the arm that has never been rendered. */
{
  const all = new Set(famNames.map((_, i) => i));
  const tailIdx = famNames.indexOf('tail');
  const exceptTail = new Set(famNames.map((_, i) => i).filter((i) => i !== tailIdx));
  const arms = [['A-ship', null], ['B-tailonly', exceptTail], ['C-none', all]];
  for (const [nm, drop] of arms) {
    const r = raster({ shot: SHOT, dropTuft: drop });
    writePNG(path.join(OUT, `${TAG}-${SHOT}-${nm}.png`), r.W, r.H, toRGB(r.idBuf, r.W, r.H));
    // ink fraction over the figure's bbox: how much of what you see is hull, not fur
    let ink = 0, fig = 0;
    for (let i = 0; i < r.idBuf.length; i++) {
      const v = r.idBuf[i];
      if (v < 0) continue;
      fig++; if (v === ID_SHELL) ink++;
    }
    const hist = {};
    for (let i = 0; i < r.idBuf.length; i++) { const v = r.idBuf[i]; if (v < 0) continue; hist[v] = (hist[v] || 0) + 1; }
    const card = (hist[8] || 0) + (hist[9] || 0) + (hist[10] || 0);
    console.log('%-12s tris %6d   figure px %6d   ink px %6d   ink frac %s   CARD px %5d',
      nm, r.tris, fig, ink, (ink / Math.max(1, fig)).toFixed(4), card);
    console.log('             per-id: %s', Object.keys(hist).sort((a,b)=>a-b).map(k=>k+':'+hist[k]).join(' '));
  }
  console.log('families: %s  (tail idx %d)', famNames.join(','), tailIdx);
}
