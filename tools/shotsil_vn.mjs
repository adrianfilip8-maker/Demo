...
function render(tris, bodyV, yaw, elev, W, H, { shade = false } = {}) {
  ...
  const nrmAll = geo.attributes.normal;
  const _n3 = new THREE.Vector3();
  for (const [a, b, c, arr, mc] of tris) {
    const P = [toPx(verts.get(key(arr, a))), toPx(verts.get(key(arr, b))), toPx(verts.get(key(arr, c)))];
    let R = 18, G = 18, B = 18;
    if (shade) {
      /* SHADE FROM THE VERTEX NORMAL, not the face normal. This is the whole point of this
         variant: `Body.addTuft` biases every clump's normals 82% toward the host surface so
         the clump shades with the skin, and a face-normal rasteriser throws that away and
         reports chips that the real renderer does not draw. Rotated into view space the same
         way the positions are, so the key direction stays meaningful. */
      const nrm = (arr === bodyV) ? nrmAll : null;
      const nA = nrm ? [nrm.getX(a), nrm.getY(a), nrm.getZ(a)] : [0, 0, 1];
      const nB = nrm ? [nrm.getX(b), nrm.getY(b), nrm.getZ(b)] : [0, 0, 1];
      const nC = nrm ? [nrm.getX(c), nrm.getY(c), nrm.getZ(c)] : [0, 0, 1];
      const nx = (nA[0] + nB[0] + nC[0]) / 3, ny = (nA[1] + nB[1] + nC[1]) / 3, nz = (nA[2] + nB[2] + nC[2]) / 3;
      _n3.set(nx, ny, nz).normalize();
