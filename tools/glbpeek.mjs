...
(g.images || []).forEach((im, i) => {
  ...
  if (!/png/.test(mime)) { console.log(`  [${i}] ${name.padEnd(22)} ${mime} ${(bytes.length / 1024).toFixed(0)} KB  (not decoded here)`); return; }
  ...
  } catch (e) { console.log(`  [${i}] ${name} — decode failed: ${e.message}`); }
});
...
const all = [];
(g.meshes || []).forEach((m) => m.primitives.forEach((p) => all.push(...acc(p.attributes.POSITION))));
...
for (let s = N - 1; s >= 0; s--) {
  ...
  const xa = Math.min(...xs), xb = Math.max(...xs), za = Math.min(...zs), zb = Math.max(...zs);
