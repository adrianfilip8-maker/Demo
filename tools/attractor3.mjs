/**
 * §281 hunt / PREREG-attractor3.md — scene or PostFX, bisected at debugRaw('scene').
 *
 * Three boots (sly-closeup calibration, hero, interior), two conditions each — `composed`
 * (shipped pipeline) and `rawscene` (postfx.debugRaw(true,'scene'), the pre-tonemap scene
 * target, proven as an instrument in holdscope.mjs) — each condition an A(raw)/B(fix)
 * same-boot swap pair on the mode-faithful swap. debugRaw restored after the pair; readback
 * records pf._debugRaw/_debugSrc; C-DRIFT re-renders composed base A at the end.
 */
...
const OUT = process.env.SANDS_OUT || 'shots/attractor3';
...
const CONDS = [
  { cond: 'composed', raw: false },
  { cond: 'rawscene', raw: true },
];
...
for (const shot of SHOTS) {
  ...
  const got = await withGame({ width: 1280, height: 720, quality: 'high', timeout: 900000 },
    async ({ page }) => page.evaluate(async (args) => {
      ...
      if (!pf || typeof pf.debugRaw !== 'function') return { error: 'postfx.debugRaw missing — VOID' };
      ...
      for (const c of conds) {
        if (c.raw) pf.debugRaw(true, 'scene'); else pf.debugRaw(false);
        const modeA = await swap('raw');
        const A = await shoot();
        const readback = { debugRaw: pf._debugRaw, debugSrc: pf._debugSrc };
  ...
  for (const p of got.pairs) {
    ...
    if (p.cond === 'composed') composedA = a;
    ...
    console.log(`${shot.padEnd(12)} ${p.cond.padEnd(9)} mask ${String(n).padStart(6)} (${(100 * row.cov).toFixed(2)}%)  `
      + `hueA ${row.hueA?.toFixed(1)}°  hueB ${row.hueB?.toFixed(1)}°  swing ${swing?.toFixed(1)}°  `
      + `rb raw=${p.readback.debugRaw}/${p.readback.debugSrc}`);
  }

  const drift = png(got.drift, `${OUT}/${shot}-drift-A.png`);
  const d0 = readPNG(composedA.file), d1 = readPNG(drift.file);
  let leaked = 0;
  ...
  results.push({ shot, cond: 'DRIFT', leaked, sha: drift.sha, vsSha: composedA.sha });
  writeFileSync(ARMS, JSON.stringify(results, null, 1));
  console.log(`${shot.padEnd(12)} DRIFT     ${leaked} px >= ${FLOOR} vs composed A  ${leaked === 0 ? 'CLEAN' : 'LEAKED'}`);
}

console.log('\nscore per PREREG-attractor3 §4 (S = R_rawscene(shot) / R_rawscene(sly-closeup))');
