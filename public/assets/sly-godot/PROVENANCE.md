...
|---|---|---|
| `Idle Fight 1` | `Library Sly MASTER 005.res` | 4.67 s standing fight guard — forward-leaning ready stance (torso 30.0° off vertical), 18.3 cm of lateral hip weight shift in the frame the player sees — the largest side-to-side travel of any idle measured, though NOT, as a first draft of this line claimed, 2.9× `Idle Anim 1`: that clip's take is authored 54.2° off-axis and §715's `centerYaw` re-base rotates most of its shift from side-to-side into fore-aft (re-extracted with `centerYaw` off it reads 16.1 sway / 6.1 surge, reproducing §715's published 16.2; with it on, 6.4 / 15.3). In the clips' own frames the two are a near tie — feet grounded (lowest contact −4.7 cm against the rig's bind foot), toe lift 1.6/1.4 cm (an idle, not a gait), loop seam 0.1° at `neck`. **Bound to `idle_bored`** — the reference authored no bored idle anywhere, and this is the corpus's one remaining unbound animated standing idle. |
